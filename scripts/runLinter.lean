...
/--
Usage : `runLinter [--update] [Time.Calendar.Days]`

Runs the linters on all declarations in the given module (or `Time` by default).
...
unsafe def main (args : List String) : IO Unit := do
  ...
  let some module :=
      ...
      | _ => none
    | IO.eprintln "Usage : runLinter [--update] [Time.Calendar.Days]" *> IO.Process.exit 1
  ...
  withImportModules [{module}] {} (trustLevel := 1024) fun env =>
    ...
    Prod.fst <$> (CoreM.toIO · ctx state) do
      let decls ← getDeclsInPackage `Time
      ...
      if failed then
        let fmtResults ←
          formatLinterResults results decls (groupByFilename := true)
            "in Time" (runSlowLinters := true) .medium linters.size
